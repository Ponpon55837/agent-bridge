...
tmux-bridge-mcp 需要 Node.js 18+ 與 npx。為降低 npm 供應鏈風險，本專案將已審查版本記錄在 `package.json` 與 `package-lock.json`；請使用 lockfile 所記錄的版本，不要直接使用未鎖定版本：

~~~bash
npx -y tmux-bridge-mcp@0.3.0 setup
npx -y tmux-bridge-mcp@0.3.0 --help
~~~

目前審查版本為 `0.3.0`；升級版本前請重新審查套件來源、integrity 與變更內容。
